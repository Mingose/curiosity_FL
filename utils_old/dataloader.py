...
def load_data(data_root, dataset, phase, batch_size, top_k_class=None, sampler_dic=None, num_workers=4, shuffle=True, cifar_imb_ratio=None):
    ...
    # template = './label_data/%s/%s'%(dataset, dataset)
    txt = data_root+"/"+'%s.txt'%(phase)  
    # template = '/media/pi/games/datasat/imagnet/save_data' 
    # img_path =  '/media/pi/games/datasat/imagnet'    

    if dataset == "imageNet_LT":
        template = 'data/imageNet_LT/save_data' 
        img_path =  'data/imageNet_LT/raw_data' 
    else:
        template = '/media/yang/Linux_data_EXT4/Casual_fedavg/data/meter/save_data' 
        img_path =  '/media/yang/Linux_data_EXT4/Casual_fedavg/data/meter/train'
    ...
    if dataset == 'CIFAR10_LT':
        ...
        set_ = LT_Dataset(img_path, txt, transform, template=template, top_k=top_k_class)
